...
def get_args():
    ...
    parser.add_argument('-an' , "--ancho"  , help="Ancho imagen"          , default=960, type=int)
    parser.add_argument('-al' , "--alto"  , help="Alto imagen"          , default=640, type=int)
    parser.add_argument('-cP' , "--critPredic"  , help="Criterio para maximo score de prediccion"          , default=0.65, type=float)
...
def NMS(ancho,alto,coordenadas,coordenadas2,scores,preds,th,visualize,frame):
    ...
    while len(idxs) > 0:
        ...
        for pos in range(0, last):
            ...
            # Itero sobre todos los BB que hay
            j = idxs[pos]
            #print ("Analizando traslape de imagen",x1[i],y1[i],"con",x1[j],y1[j])
            # Encuentro coordenadas para el traslape
            xx1 = max(x1[i], x1[j])
            yy1 = max(y1[i], y1[j])
            xx2 = min(x2[i], x2[j])
...
def escribirReporte(therNMS,therDIF,VP,FP,FN,P,R,F1,folder,bandera):
    if bandera == 0:
        print ("Se escribio el reporte de esta escena")
...
if __name__ == '__main__':
    ...
    scores1,preds1 = predecir(classifier2,carHOG,numimag)
    ...
    if args.visualize == 1:
        plt.show();
    #escribirReporte(args.thereshold,args.diferencia,len(verPositivos),len(falPositivos),falNegativos,pre,re,f1,'C:/Deteccion_Conteo_Personas_Escenas_PUJ-1703/Recursos/ModelosClasificacion/Reporte/ReporteHOGRF.csv',args.visualize)

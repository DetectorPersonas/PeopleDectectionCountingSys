...
def load_gmm(folder,version):
    means = np.load(folder + "MeansRF"  + version + ".npy")
    covs = np.load(folder + "CovsRF" + version + ".npy")
    priors = np.load(folder + "PriorsRF" + version + ".npy")
...
def NMS(ancho,alto,coordenadas,scores,preds,th,visualize,frame):
    ...
    for i in range(0,len(boxes)):
        scor = scores[i]
        personaScore = scor[0,0]

        if preds[i] == 0 and personaScore > args.critPredic:
...
if __name__ == '__main__':
    ...
    #classifier1 = joblib.load(args.dir + "SVMFisher" + "4500" + ".pkl")
    classifier2 = joblib.load(args.dir + args.version)

    # Evaluacion de clasificadores
    scores1,preds1 = predecir(classifier2,fisherVectors,numimag)
    ...
    if args.visualize == 1:
        plt.show();
    #escribirReporte(args.thereshold,args.diferencia,len(verPositivos),len(falPositivos),falNegativos,pre,re,f1,'C:/Deteccion_Conteo_Personas_Escenas_PUJ-1703/Recursos/ModelosClasificacion/Reporte/ReporteSIFTFVRF.csv',args.visualize)
